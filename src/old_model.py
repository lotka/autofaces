...
def cnn(config):
    ...
    if config['network'] == 'encoder_fc_1':
        network.append(nn_layer(ll(network), int(ll(network).get_shape()[1]), 1000, 'fully_connected_1',config, act=leaky_relu))
        network.append(nn_layer(ll(network), int(ll(network).get_shape()[1]), 500, 'fully_connected_2', config,act=leaky_relu))
    ...
    decoder = []
    if use_autoencoder:
        if config['autoencoder']['decoder'] == 'gudi_test_network_1':
            # ll(network,2) means start the decoder from the second to last bit of the autoencoder
            decoder.append(dcnn_layer(ll(network), [5, 5, 1, 64], x_.get_shape(), 'VALID', 'Deconvolution_1', config, config,act=tf.nn.sigmoid))
        elif config['autoencoder']['decoder'] == 'gudi_test_network_2':
            decoder.append(dcnn_layer(ll(network,-2), [5, 5, 1, 64], x_.get_shape(), 'VALID', 'Deconvolution_1', config, config,act=tf.nn.sigmoid))
        elif config['autoencoder']['decoder'] == 'gudi_test_network_3':
            decoder.append(dcnn_layer(ll(network), [5, 5, 64, 64], [batch_size,22,22,64], 'VALID', 'Deconvolution_1', config))
            print 'Resize:', [batch_size,22,22,64], '-->',[batch_size,43,43,64]
            decoder.append((tf.image.resize_images(ll(decoder), 43, 43),None))
            decoder.append(dcnn_layer(ll(decoder), [5, 5, 1, 64], [100,47,47,1], 'VALID', 'Deconvolution_2', config,act=tf.nn.sigmoid))
    ...
    """
    Autoencoder if branching off from the fc layer
    """

    if use_autoencoder:
        if config['autoencoder']['decoder'] == 'fully_connected_1':
            decoder.append(nn_layer(ll(network), int(ll(network).get_shape()[1]),2209, 'decoder_fully_connected_1', config, act=leaky_relu))

        y_auto = ll(decoder)
        y_image = tf.reduce_mean(tf.reshape(y_auto, shape=shape_2),3,keep_dims=True)
