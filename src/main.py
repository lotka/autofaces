...
def main(path,config_overwrite=None):
    ...
    config = PyExp(config_file='config/cnn.yaml', path=path, config_overwrite=None)
    if config['data']['dataset'] == 'disfa':
        import disfa
...
def run_experiment(args,config_overwrite=None):
    ...
    test_set_analysis.main((sys.argv[0], data_path,'final', args.device), data=data, overwrite_dict=overwrite_dict)

    data.train.batch_counter = 0
    tf.reset_default_graph()
    test_set_analysis.main((sys.argv[0], data_path, 'early', args.device), data=data, overwrite_dict=overwrite_dict)
